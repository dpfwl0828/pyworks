...
# variable = 변수
ck_val = tk.BooleanVar() # True 나 False를 저장한다 
ck_val.set(False )
...
for i in range(n): # 0부터 n-1 까지 반복 
    ck_val[i] = tk.BooleanVar() # True 나 False를 저장한다 
    ck_val[i].set(False)
    ck_btn[i] = tk.Checkbutton(window, text=hobby[i], font=("System", 13),
            variable=ck_val[i])
    # 좌표로 이동하는 함수 - place(x, y) # x = ~~ , y = ~~ 값을 지정해줘야 함 
    ck_btn[i].place(x = 100, y = 20 + (30 * i)) # y : 20, 50, 80, 110

# 확인 버튼 
tk.Button(window, text="확인", command=click).place(x = 120, y = 160)

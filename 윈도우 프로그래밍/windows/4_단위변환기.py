...
def convert():
    try:
        # 입력 상자의 자료형은 문자형 이므로 숫자로 변경해야함 
        byte_mb = int(ent_mb.get())
        out_kb.delete(0.0, END) # 이전 입력문자 삭제 
        byte_kb = byte_mb * 1024 # 변환상수 곱하기 
        out_kb.insert(END, byte_kb)
    except ValueError: # 숫자가 아닌 문자를 입력하면 오류 처리 
...
# 변환 버튼 
Button(frame, text="변환", command = convert).grid(row=2, columnspan = 2) # 셀 병합 
